...
root = Tk()
root.title("SJCEM Railway Reservation")
...
my_image = PhotoImage(file='sjcemrail.gif')
my_img = canvas.create_image(0, 0, anchor=NW, image=my_image)

Label(root, text="SJCEM RAILWAYS",font=('Slab Serif',17),bg='Orange').place(x=100,y=150)
...
def printMsg():
    if((entry_1.get()=='chaitanya' and entry_2.get()=='123') or (entry_1.get()=='akash' and entry_2.get()=='456') or (entry_1.get()=='aniket' and entry_2.get()=='789')or (entry_1.get()=='prajual' and entry_2.get()=='101') ):
            ...
            if(len1==10):
                if(int(date1)<=31 and int(date1)>=1):
                    if(int(month1)>=1 and int(month1)<=12):
                        if(int(year11)>=2021 and int(year11)<=2021):
                            window.destroy()
                            Search()
                        else:
